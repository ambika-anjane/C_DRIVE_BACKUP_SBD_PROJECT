{{ config(
   ...
   pre_hook= 'create sequence if not exists public.promo_code as int
   start 1
   increment 1;'
)
}}
...
select  distinct
    ...
    from sq_promo s 
    {% if is_incremental() %}
    left outer join lkp_dim_promo l 
    on l."promo_code" = s."promo_code"
    where l."promo_key" is null
    {% endif %}
