...
class genericAPI(object):
    ...
    def get_Cases_Data(cls, dateslist, base_url):
       ...
       for datestring in dateslist:
          ...
          try:
             ...
             print(df_final_downloaded_csv.columns)
          except Exception as e:
             raise CustomException(e)
             sys.exit(1)
    ...
    def getFredSeriesUnits(cls):
        ...
        snowflakeTableName = 'FRED_SERIES_UNITS'  
        ...
        print(df_fredSeriesUnits)
        return df_fredSeriesUnits
    
    @classmethod
    def getIHSNotFoundLinks(cls):
        snowflakeAccount = 'sbd_caspian.us-east-1'
        snowflakeUser = 'DEV_INGEST'
        snowflakePass = 'poonooD9fooBeth9'
        snowflakeWarehouse = 'DEV_INGEST_WH'
        ...
        query_text = "SELECT SERIES_JSON_DATA FROM DEV_RAW.IHSMARKIT.IHS_PRICING_AND_PURCHASING_HISTORICAL_SERIES_LANDING P where P.observations_json_data  like '%<%'"
        ##query_text = "SELECT SERIES_JSON_DATA FROM DEV_RAW.IHSMARKIT.TEST_04232022 P where P.observations_json_data  like '%<%'"
        ...
        cursor.execute(query)
        print('testing 8')
        df_ihsConnectionLinks = pd.DataFrame.from_records(iter(cursor), columns=[x[0] for x in cursor.description])
        
        listValue= []
        for (key, value) in df_ihsConnectionLinks.iteritems():
            for val in value:
                dictVal = ast.literal_eval(val)
                dictVal = dictVal.copy()
                listValue.append(dictVal)
